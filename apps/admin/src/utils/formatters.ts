...
export function formatOrderSource(source?: string, requestId?: string | boolean, platform?: string) {
  ...
  if (/^web[_-]/.test(requestKey)) return 'Web 下单'
  if (/^(api|member-api|member_api)$/.test(normalized)) return 'API 下单'
  if (/^(web|pc)$/.test(normalized)) return 'Web 下单'
  if (/^(h5|mobile|wap)$/.test(normalized)) return 'H5 下单'
  if (/^api/.test(normalized)) return 'API 下单'
  if (requestId === true && !normalized) return 'API 下单'
  return '-'
}
