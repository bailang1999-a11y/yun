...
export async function updateUserGroupOrderPermission(
  ...
  payload: { orderEnabled: boolean; realNameRequiredForOrder: boolean; priceLimitEnabled: boolean; priceLimitNotice?: string }
) {
...
export async function updateUserCredentials(userId: UserAccount['id'], payload: UserCredentialPayload) {
  const { data } = await apiClient.post<unknown>(`/api/admin/users/${userId}/credentials`, payload)
...
function normalizeUserGroup(item: Record<string, unknown>): UserGroup {
  ...
  return {
    ...
    priceLimitEnabled: booleanValue(item.priceLimitEnabled, true),
    priceLimitNotice: text(item.priceLimitNotice),
