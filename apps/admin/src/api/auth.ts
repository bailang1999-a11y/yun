...
export async function createAdminSliderToken() {
  const { data } = await apiClient.post<unknown>('/api/admin/auth/slider', { terminal: 'admin' })
  return text(unwrapValue(data as ApiEnvelope<string>))
}
