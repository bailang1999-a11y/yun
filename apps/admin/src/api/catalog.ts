...
export async function reorderCategories(categoryIds: Category['id'][]) {
  const { data } = await apiClient.post<unknown>('/api/admin/categories/reorder', { categoryIds }, { timeout: CATEGORY_WRITE_TIMEOUT_MS })
