...
export async function fetchRemoteGoodsSnapshot(supplierId: GoodsIntegration['supplierId'], supplierGoodsId: string) {
  const { data } = await apiClient.get<unknown>(`/api/admin/suppliers/${supplierId}/remote-goods/snapshot`, {
    params: { supplierGoodsId }
  })

  return normalizeIntegration(unwrapValue<Record<string, unknown>>(data as ApiEnvelope<Record<string, unknown>>))
}
...
function normalizeGoodsPayload(payload: GoodsCreatePayload) {
  const priceLimitText = payload.priceLimitText?.trim() || ''
  return {
    name: payload.name,
    ...
    benefitBrand: payload.benefitBrand || '',
    tags: payload.tags || [],
    priceLimited: Boolean(priceLimitText),
    priceLimitText,
...
function normalizeGoods(item: Record<string, unknown>): Goods {
  ...
  const priceLimitText = text(item.priceLimitText ?? item.price_limit_text)
  ...
  return {
    ...
    benefitType: text(item.benefitType),
    benefitBrand: text(item.benefitBrand),
    tags: normalizeTagList(item.tags),
    priceLimited: Boolean(priceLimitText || item.priceLimited),
    priceLimitText: priceLimitText || (item.priceLimited ? '限价' : ''),
...
function normalizeIntegrations(value: unknown) {
  ...
  return value.map((item) => normalizeIntegration((item || {}) as Record<string, unknown>))
}
...
function normalizeIntegration(record: Record<string, unknown>): GoodsIntegration {
  return {
    id: text(record.id),
