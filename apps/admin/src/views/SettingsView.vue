  ...
  <section class="settings-grid" v-loading="loading">
        ...
        <el-form-item label="站点 Logo">
          <div class="logo-uploader">
            ...
            <div class="logo-actions">
              <el-upload accept="image/png,image/jpeg,image/webp,image/gif" :show-file-list="false" :before-upload="handleLogoUpload">
                <el-button :icon="ImageUp">上传本地 Logo</el-button>
              </el-upload>
              <el-button :icon="X" :disabled="!form.logoUrl" @click="clearLogo">清除</el-button>
              <p>支持 PNG、JPG、WEBP、GIF，建议使用透明背景，大小不超过 2MB。</p>
            </div>
          </div>
        </el-form-item>
    ...
    <article class="panel">
      <div class="panel-head">
        <div>
          <h2>支付与退款</h2>
          <span>余额支付可直接使用，微信/支付宝需完成真实网关接入后启用</span>
        </div>
      </div>
      ...
      <el-form :model="form" label-position="top" class="settings-form">
        ...
        <el-form-item label="注册类型">
          <el-radio-group v-model="form.registrationType" class="register-type-group">
            <el-radio-button value="MOBILE">手机号</el-radio-button>
            <el-radio-button value="EMAIL">邮箱</el-radio-button>
            <el-radio-button value="FREE">自由注册</el-radio-button>
          </el-radio-group>
          <p class="form-help">手机号注册使用短信验证码；邮箱注册和自由注册不触发短信验证码，仍会按人机验证配置校验。</p>
        </el-form-item>
