...
class PersistentOrderStoreTest {
    ...
    @Test
    void deleteOrderDataRemovesOrderAndLinkedRecords() {
        when(orderRecordMapper.findIdByOrderNo("ORD-1")).thenReturn(101L);
        when(orderRecordMapper.hardDeleteByOrderNo("ORD-1")).thenReturn(1);

        boolean deleted = store.deleteOrderData("ORD-1");

        assertThat(deleted).isTrue();
        verify(paymentCallbackLogMapper).hardDeleteByOrderNo("ORD-1");
        verify(cardRecordMapper).releaseByOrderId(101L);
        verify(refundRecordMapper).hardDeleteByOrderId(101L);
        verify(paymentRecordMapper).hardDeleteByOrderNo("ORD-1");
        verify(orderRecordMapper).hardDeleteDeliveryTasksByOrderNo("ORD-1");
        verify(orderRecordMapper).hardDeleteStatusLogsByOrderNo("ORD-1");
        verify(orderRecordMapper).hardDeleteByOrderNo("ORD-1");
    }
