...
public class ConfigPersistenceStore {
    ...
    @Transactional
    public void saveRuntimeSetting(String key, String value) {
        systemSettingRecordMapper.upsertSetting(toSystemSettingRecord(key, value));
    }

    public Map<String, String> encryptSecretForSetting(String secret) {
        CardCipherService.EncryptedCard encrypted = cardCipherService.encrypt(secret == null ? "" : secret);
        return Map.of(
    ...
    @Transactional
    public int migrateLegacySupplierApiKeys() {
        int migrated = 0;
        for (SupplierRecordEntity legacy : supplierRecordMapper.selectLegacyPlaintextApiKeySnapshots()) {
            String plaintextApiKey = legacy.getApiKey();
            if (!StringUtils.hasText(plaintextApiKey)) {
                continue;
            }
            CardCipherService.EncryptedCard encrypted = cardCipherService.encrypt(plaintextApiKey);
            legacy.setApiKey(null);
            legacy.setApiKeyCiphertext(encrypted.ciphertext());
            legacy.setApiKeyNonce(encrypted.nonce());
            legacy.setApiKeyKeyVersion(encrypted.keyVersion());
            legacy.setApiKeyHash(encrypted.hash());
            legacy.setApiKeyMasked(maskSecret(plaintextApiKey));
            migrated += supplierRecordMapper.migrateApiKeyToEncrypted(legacy);
        }
        return migrated;
    }
    ...
    public void deleteGoodsChannel(Long id) {
        goodsChannelRecordMapper.hardDelete(id);
    }

    @Transactional
    public void deleteGoodsChannelsByGoods(Long goodsId) {
        goodsChannelRecordMapper.hardDeleteByGoods(goodsId);
    }
    ...
    private SupplierRecordEntity toSupplierRecord(SupplierItem item) {
        ...
        if (StringUtils.hasText(item.apiKey())) {
            CardCipherService.EncryptedCard encrypted = cardCipherService.encrypt(item.apiKey());
            entity.setApiKeyCiphertext(encrypted.ciphertext());
            entity.setApiKeyNonce(encrypted.nonce());
            entity.setApiKeyKeyVersion(encrypted.keyVersion());
            entity.setApiKeyHash(encrypted.hash());
        }
    ...
    private SupplierItem toSupplierItem(SupplierRecordEntity entity) {
        String apiKey = decryptedSupplierApiKey(entity);
        return new SupplierItem(
            ...
            apiKey, entity.getApiKeyMasked(), entity.getCallbackUrl(), entity.getTimeoutSeconds(),
    ...
    private UserGroupRecordEntity toUserGroupRecord(UserGroupItem item) {
        ...
        entity.setPriceLimitEnabled(item.priceLimitEnabled());
        entity.setPriceLimitNotice(item.priceLimitNotice());
        return entity;
    }
    ...
    private UserGroupItem toUserGroupItem(UserGroupRecordEntity entity, List<GroupRuleItem> rules) {
        return new UserGroupItem(
            ...
            Boolean.TRUE.equals(entity.getRealNameRequiredForOrder()),
            entity.getPriceLimitEnabled() == null || entity.getPriceLimitEnabled(),
            entity.getPriceLimitNotice(),
            rules
        );
    }
