...
public class PersistentOrderStore {
    ...
    @PostConstruct
    public void ensureOrderSnapshotSchema() {
        if (orderRecordMapper.countOrderColumn("delivery_message") == 0) {
            orderRecordMapper.ensureDeliveryMessageColumn();
        }
        if (orderRecordMapper.countOrderColumn("delivery_items_json") == 0) {
            orderRecordMapper.ensureDeliveryItemsColumn();
        }
        if (orderRecordMapper.countOrderColumn("channel_attempts_json") == 0) {
            orderRecordMapper.ensureChannelAttemptsColumn();
        }
        if (orderRecordMapper.countOrderColumn("order_ip") == 0) {
            orderRecordMapper.ensureOrderIpColumn();
        }
        if (orderRecordMapper.countOrderColumn("order_ip_location") == 0) {
            orderRecordMapper.ensureOrderIpLocationColumn();
        }
    }
