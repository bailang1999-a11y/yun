...
public interface GoodsChannelRecordMapper extends BaseMapper<GoodsChannelRecordEntity> {
    ...
    @Update("UPDATE goods_channels SET deleted_at = CURRENT_TIMESTAMP(3) WHERE goods_id = #{goodsId}")
    int softDeleteByGoods(@Param("goodsId") Long goodsId);

    @Delete("DELETE FROM goods_channels WHERE id = #{id}")
    int hardDelete(@Param("id") Long id);

    @Delete("DELETE FROM goods_channels WHERE goods_id = #{goodsId}")
