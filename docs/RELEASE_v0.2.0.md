...
全新数据卷可直接启动。已有数据库升级前先备份，并在启动新后端前执行：
...
- `XIYIYUN_HTTP_BIND`，生产保持 `127.0.0.1` 并通过 HTTPS 反向代理对外访问
...
- 后端 Docker 内 `mvn test`：通过，57 个测试通过。
- `docker compose -p xiyiyun -f docker-compose.prod.yml --env-file .env.feiniu.example config`：仅验证 Compose 结构通过；`.env.feiniu.example` 仍含占位值，不能作为生产配置。
- `npm run prod:preflight` 使用脱敏生产格式环境变量：通过。
